...
def decide(req: DecideRequest):
    if not req.community_cards:
        is_fresh_headsup_open = (req.num_active_players == 2 and req.to_call == 0 and not req.action_history)
        if is_fresh_headsup_open:
            action, raise_amount = predictor.predict_headsup_opening_action(
